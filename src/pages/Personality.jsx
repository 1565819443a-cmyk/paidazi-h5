...
export default function Personality() {
  ...
  useEffect(() => {
    const saved = profile?.personality_answers?.q1 ? profile.personality_answers : getStorageSync('personality');
    if (saved && saved.q1) {
  ...
  async function submitTest() {
    ...
    try {
      await saveProfile({ personality_answers: answers });
    } catch (err) {
      console.error('保存性格测试失败:', err);
    }
