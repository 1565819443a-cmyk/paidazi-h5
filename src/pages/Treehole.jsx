...
export default function Treehole() {
  ...
  async function loadPosts() {
    ...
    try {
      ...
      setPosts((data || []).filter((item) => item.category === 'treehole'));
  ...
  return (
    <div className="community-container">
      <div className="comm-header">
        <span className="comm-title">💭 树洞</span>
        <span className="comm-subtitle">匿名倾诉你的心事</span>
      </div>
      ...
      <div className="publish-card" onClick={() => navigate('/publish')}>
        ...
        <span className="publish-placeholder">匿名说点什么...</span>
        <div className="publish-btn">发布</div>
      </div>

      {loading && <div className="empty"><span>加载中...</span></div>}
      {!loading && posts.length === 0 && <div className="empty"><span>暂无树洞内容</span></div>}
        ...
        {posts.map((item) => (
          <div key={item.id} className="post-item" onClick={() => navigate(`/comment?postId=${item.id}`)}>
            <div className="post-header">
              ...
              <div className="post-cat treehole">{item.category_name}</div>
            </div>
