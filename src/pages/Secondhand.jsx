...
export default function Secondhand() {
  ...
  async function loadPosts() {
    ...
    try {
      ...
      setPosts((data || []).filter((item) => item.category === 'secondhand'));
  ...
  return (
    <div className="community-container">
      <div className="comm-header">
        <span className="comm-title">🛒 二手交易</span>
        <span className="comm-subtitle">闲置物品交易</span>
      </div>
      ...
      <div className="publish-card" onClick={() => navigate('/publish')}>
        ...
        <span className="publish-placeholder">发布闲置物品...</span>
        <div className="publish-btn">发布</div>
      </div>

      {loading && <div className="empty"><span>加载中...</span></div>}
      {!loading && posts.length === 0 && <div className="empty"><span>暂无二手物品</span></div>}
        ...
        {posts.map((item) => (
          <div key={item.id} className="post-item" onClick={() => navigate(`/comment?postId=${item.id}`)}>
            <div className="post-header">
              ...
              <div className="post-cat secondhand">{item.category_name}</div>
            </div>
