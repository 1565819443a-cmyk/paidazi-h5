...
export default function AiChat() {
  const [messages, setMessages] = useState(() => {
    try {
      const saved = sessionStorage.getItem('aichat_history');
  ...
  useEffect(() => {
    try {
      sessionStorage.setItem('aichat_history', JSON.stringify(messages.slice(-MAX_HISTORY)));
    } catch {}
  }, [messages]);
  ...
  async function sendMessage(text) {
    ...
    try {
      ...
      const reply = await aiChat('companion', history, (streamed) => {
        setMessages((prev) => {
          const updated = [...prev];
          updated[updated.length - 1] = { role: 'ai', content: streamed };
          return updated;
        });
      });
      ...
      setMessages((prev) => {
        const updated = [...prev];
        updated[updated.length - 1] = { role: 'ai', content: '哎呀，我好像卡住了😵 可能是网络问题或API超时，请稍后再试～' };
        return updated;
      });
    } finally {
      setLoading(false);
  ...
  function clearHistory() {
    ...
    sessionStorage.removeItem('aichat_history');
  }
